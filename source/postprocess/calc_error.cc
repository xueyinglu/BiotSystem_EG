...
void BiotSystem::calc_error()
{
    ...
    MandelPressure mandel_p(t);
    TerzaghiPressure terzaghi_p(t);
    double L2_p_EG = 0;
    ...
    for (; cell != endc; ++cell)
        if (cell->is_locally_owned())
        {
            ...
            {
                terzaghi_p.value_list(fe_values_pressure.get_quadrature_points(), exact_p_values);
            }
            ...
            for (unsigned int q = 0; q < n_q_points; ++q)
            {
                ...
                L2_p_EG += (exact_p_values[q] - sol_p_values[q][1] - sol_p_values[q][0]) * (exact_p_values[q] - sol_p_values[q][1] - sol_p_values[q][0]) * fe_values_pressure.JxW(q);
            }
        }
